...
from web_ui.time_sharing import get_index_data
...
class TradeStructure(CoreTradeLogic):
    ...
    def run_one_stock_once(self, code_name, indicators_config=None):
        ...
        # if "sz." in code_name:
        #     codeStr = code_name.replace("sz.", "")
        # elif "sh." in code_name:
        #     codeStr = code_name.replace("sh.", "")

        data_path = os.path.join(f"Data/Baostock/day/{code_name}_{self.config.START_STAMP}_{self.config.END_STAMP}.csv")
        ...
        if not os.path.exists(data_path):
            ...
            rs = bs.query_history_k_data_plus(
                ...
                start_date=self.config.START_STAMP,
                end_date=self.config.END_STAMP,
                frequency="d",
                adjustflag="1",
            )
            # 打印结果集
