...
def chan_show_base():
    symbol = st.sidebar.text_input(
        "stock code,上证指数:sh000001,深证成指:sz399001,创业板指:sz399006,沪深300:sz399300,中证500:sh000905",
        "000001",
    )
    ...
    if symbol in ["sh000001", "sz399001", "sz399006", "sz399300", "sh000905"]:
        ...
        df = ak.stock_zh_a_hist_min_em(
            symbol=symbol, period=period.replace("min", ""), adjust=adjust
        )
    ...
    if end_date is not None:
        try:
            df = df[pd.to_datetime(df["日期"]) < end_date]
        except Exception as e:
            print(e)
            print(df.columns)
